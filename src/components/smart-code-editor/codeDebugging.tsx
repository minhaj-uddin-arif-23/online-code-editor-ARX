...
import { sampleBuggyCode, sampleCode } from "@/lib/examples";
import { HistoryItem } from "@/types";
import React, { useState } from "react";
interface CodeDebuggingPage {
  addTohistory: (
...
export default function CodeDebugging({ addTohistory }: CodeDebuggingPage) {
  ...
  const handleDebug = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setDebug(debuggingText);
        addTohistory("debug", code, debuggingText);
      } else {
        setDebug(`Error : ${data.error}`);
      }
    } catch (error) {
      console.log(error);
      setDebug("Failed to fetch Debugging.Please try again");
    } finally {
      setLoading(false);
    }
  };
  const insertSampleCode = () => {
    ...
    setError("Tpyerror : Code you given can not read propertise of undefined");
  };
  return (
    <div className="p-6 bg-[#0F172A] border border-white/10 rounded-xl space-y-5">
      {/* Header */}
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-white">Debug Code</h2>
      ...
      <div className="space-y-2">
        <div className="relative">
          ...
          <textarea
            ...
            placeholder="Paste your debugging code here..."
          ></textarea>
        </div>
      </div>
      <div className="relative">
        {/* Code editor box */}
        <label htmlFor="">Explain message (optional)</label>
        <textarea
          id="error"
          rows={2}
          className="w-full p-4 bg-[#1E293B] text-white/90 rounded-lg border border-white/10 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none font-mono text-sm resize-none shadow-inner"
      ...
      >
        {loading ? (
          <div className="flex items-center gap-2 text-white">
            ...
            <span className="text-sm">Debugging Code...</span>
          </div>
        ) : (
